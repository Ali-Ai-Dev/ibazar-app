...
public class JalaliCalendar extends Calendar {
    ...
    public static String getGregorianDate(String jalaliDate) {
        ...
        Date dt = cal.getTime();
        String date = gDate.getYear() + "-" +
            fixLen(1 + gDate.getMonth()) + "-" + fixLen(gDate.getDate());
        return date;
    }

    private static String fixLen(int i) {
        if (i > 9)
            return "" + i;
        return "0" + i;
    }
    ...
    public static String gregorianToJalali(String gDate) {
        YearMonthDate gregorian = new YearMonthDate(Integer.parseInt(gDate.split("/")[0]),
            Integer.parseInt(gDate.split("/")[1]), Integer.parseInt(gDate.split("/")[2]));
        if (gregorian.getMonth() > 11 || gregorian.getMonth() < -11) {
            throw new IllegalArgumentException();
        }
        int jalaliYear;
        ...
        String res = fixLen(jalaliYear) + "/" + fixLen(jalaliMonth) + "/" + fixLen(jalaliDay);
        return res;
    }
